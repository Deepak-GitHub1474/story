...
One-to-one voice calls over WebRTC. The audio goes **phone to phone** — the
server only carries the signalling that sets the call up. A call rings for 45
seconds, reaches a phone whose app is closed, and its history is kept per
conversation for 30 days.

Direct connections fail behind carrier-grade NAT, which is most mobile data, so
those calls relay through coturn. The API mints a relay username and password
on every `POST /v1/calls`, valid five minutes — the relay secret stays on the
server, never in the APK, which is public. Setup is in
[`docs/04-voice-calling.md`](docs/04-voice-calling.md).
